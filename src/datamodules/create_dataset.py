...
resource.setrlimit(resource.RLIMIT_NOFILE, (2048*16, rlimit[1]))
...
def Train(csv,cfg,preload=True):
    ...
    for _, sub in csv.iterrows():
        ...
        if sub.mask_path != None: # if we have masks
            ...
            subject_dict['mask_orig'] = tio.LabelMap(sub.mask_path,reader=sitk_reader)
        ...
        # print("vk comment",sub.mask_path.replace("mask","seg"))
        #subject_dict['seg'] = tio.LabelMap(sub.mask_path.replace("mask","seg"),reader=sitk_reader)
    ...
    def __getitem__(self, index):
        ...
        if self.slice is not None:
            ...
            if self.cfg.get('unique_slice',False): # if all slices in one batch need to be at the same location
                ...
                self.ind = torch.randint(low,high,size=[1])
        #torch.save(subject['vol'],f"/scratch/vivek.trivedi/Vivek/Vivek/patched-Diffusion-Models-UAD/synthetic_data/original/{subject['ID']}.pt")
        #torch.save(subject['mask'],f"/scratch/vivek.trivedi/Vivek/Vivek/patched-Diffusion-Models-UAD/synthetic_data/mask/{subject['ID']}.pt")
        subject['ind'] = self.ind
        #for i in range(200):
            #try:
                #ind=random.randint(1, 380)
                #subject1 = self.ds.__getitem__(ind)
                #subject["fp"]=subject1['vol']
                #break
            #except:
        subject["fp"]=subject['vol']
                #print(ind)
        #count=0
        #Flag=True
        #while Flag:
         #   count+=1
          #  self.ind = torch.randint(low,high,size=[1])
           # if torch.sum(subject['seg'].data[...,self.ind]).item()==0:
            #    Flag=False
            #if count>150:
             #   print(subject['ID'])
             #   Flag=False
        ...
        #subject['fp'].data = subject['fp'].data[...,self.ind]
        #print("data shape",subject['vol'].data.shape)
        #print("index",self.ind)
        #subject['seg'].data = subject['seg'].data[...,self.ind]
        return subject
