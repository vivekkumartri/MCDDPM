...
class DDPM_2D(LightningModule):
    ...
    def training_step(self, batch, batch_idx: int):
        # process batch
        a=8
        ind=torch.randint(0,58,size=[1])
        input = batch['vol'][tio.DATA].squeeze(-1)
        input2=input.clone().detach()
        assert input.shape[0] == 1, "Batch size must be 1"
        input = input.squeeze(0).permute(3,0,1,2)[ind:ind+a,:,:,:] 
        #mask=batch['mask'][tio.DATA].squeeze(-1)
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        loss_diff, reco = self.diffusion(input,t=self.test_timesteps-1,noise=noise,train=True)
