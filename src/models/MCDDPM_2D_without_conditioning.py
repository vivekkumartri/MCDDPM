from src.models.modules.cond_DDPM_Mcddpm_without_condition import GaussianDiffusion
...
from src.TumorGenerated.TumorGenerated import TumorGenerated
...
class DDPM_2D(LightningModule):
    def __init__(self,cfg,prefix=None):
        ...
        # Model 
        print("num_res_blocks", cfg.get('num_res_blocks',3))
        model = OpenAI_UNet(
        ...
        self.prefix = prefix
        #self.syn_mask=TumorGenerated(keys=["image", "label"], prob=0.8)
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        if self.cfg.get('noisetype') is not None:
            ...
            noise = None
        # reconstruct
        loss, reco = self.diffusion(input, box=bbox,noise=noise,train=True)
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        #syn_mask=self.syn_mask(input2.squeeze(),data_mask.squeeze()).unsqueeze(0).permute(3,0,1,2)
        ...
        for k in range(bbox.shape[1]):
            box = bbox[:,k]
            loss_diff, reco = self.diffusion(input,t=self.test_timesteps-1, box=box,noise=noise,train=False)
            if reco.shape[1] == 2:
                reco = reco[:,0:1,:,:]
