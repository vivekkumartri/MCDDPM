...
from src.TumorGenerated.utils import GaussianSmoothing
...
#    for i in range(b):
#        print(mse(x[i],y[i]))
#        diff=torch.sqrt(mse(x[i],y[i]))
        #print((1-diff).mean())
#        loss+=torch.clamp((torch.pow(-torch.log((1-normalize_tensor(diff)).mean()), 0.3)), min=0, max=2)
#    return loss/b
...
def EL_Loss(x,y):
    ...
    for i in range(b):
        ...
        #print("mse:-",diff)
        loss+=torch.clamp((torch.pow(-torch.log((1-diff).mean()), 0.3)), min=0, max=2)
    return loss/b        
# constants
def normalize_tensor(tensor):
    return (tensor - tensor.min()) / (tensor.max() - tensor.min())

def masked_image(image,patch_size):
        # get image size
        channel, height, width = image.shape
        patch_size = [patch_size, patch_size]
        image2=image.clone()
        # sample random box
        x_min = np.random.randint(0, width -patch_size[0])
        y_min = np.random.randint(0, height -patch_size[1])
        x_max = x_min + patch_size[0]
        y_max = y_min + patch_size[1]
        for i in range(image.shape[0]):
                image2[i, y_min:y_max, x_min:x_max]= torch.zeros((patch_size[0],patch_size[1]),dtype=image.dtype,requires_grad=image.requires_grad).to(image.device)
        return [x_min,x_max,y_min,y_max],image2
def pixel_channel(original_tensor):
    b,ch,h,w=original_tensor.shape
    original_tensor=255*original_tensor
    num_channels = 256
    new_tensor = torch.zeros((b,ch,h, w, num_channels), dtype=torch.float32).to(original_tensor.device)
    for i in range(b):
        for channel in range(num_channels):
            lower_bound = channel
            upper_bound = channel + 1
            new_tensor[i,0,:, :, channel] = torch.where((original_tensor[i,0,:,:] >= lower_bound) & (original_tensor[i,0,:,:] < upper_bound),
                                                    original_tensor[i,0,:,:], torch.tensor(0.0))/upper_bound
    return new_tensor

def scale_to_0_255(tensor):
    type1=tensor.dtype
    min_vals= torch.min(tensor)
    max_vals= torch.max(tensor)

    scaled_tensor = 255.0 * (tensor - min_vals.item()) / (max_vals.item() - min_vals.item())
    
    return torch.clamp(scaled_tensor.round(), 0, 255).to(type1)
def diceloss(pred, target):
    smooth = 1.
    pred = pixel_channel(pred)
    target = pixel_channel(target)
    dice = 0
    for i in range(pred.shape[-1]):
        iflat = pred[:,:,:,:,i].contiguous().view(-1)
        tflat = target[:,:,:,:,i].contiguous().view(-1)
        intersection = (iflat * tflat).sum()
        A_sum = torch.sum(iflat * iflat)
        B_sum = torch.sum(tflat * tflat)
        dc = (1 - ((2. * intersection + smooth) / (A_sum + B_sum + smooth) )) / (torch.sum(target) + smooth)
        dice = dice + dc
    return dice

ModelPrediction =  namedtuple('ModelPrediction', ['pred_noise', 'pred_x_start'])

def create_ellipsoid_masks(height, width, num_masks): #vivek
    masks = []
    for _ in range(num_masks):
        y, x = np.ogrid[:height, :width]
        center_x, center_y = np.random.randint(0, width), np.random.randint(0, height)
        mask = ((x - center_x) / (width/2))**2 + ((y - center_y) / (height/2))**2 <= 1
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.full_noisy_conv = nn.Sequential(ResnetBlock(1,128),
                                             ResnetBlock(128,256))
        self.denoise_conv = nn.Sequential(ResnetBlock(256,128),
                                           ResnetBlock(128,1,groups=1))
        self.maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        #self.down_vi1=nn.Conv2d(in_channels=1, out_channels=128, kernel_size=3, padding=1,stride=2)
        #self.down_vi2=nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, padding=1,stride=2)
        #self.up_vi1=nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=4, padding=1,stride=2)
        #self.up_vi2=nn.ConvTranspose2d(in_channels=128, out_channels=1, kernel_size=4, padding=1,stride=2)
        self.cfg = cfg
        ...
        #self.gaussian_filter=GaussianSmoothing(channels=1).to('cuda')
        ...
        register_buffer('one_minus_sqrt_alphas_cumprod', 1. - torch.sqrt(alphas_cumprod))
    ...
    def p_losses(self, x_start, t,train, cond = None, noise = None, box=None, scale_patch=1, onlybox=False, mask=None):
        b, c, h, w = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start)) # some noise with zero mean and unit variance
        #torch.save(x_start,f"x_start_{box[0,:]}_{t[0]}.pt")
        x = self.q_sample(x_start = x_start, t = t, noise = noise) # generate a noisy image from the start image at timestep t
        #torch.save(x,f"noise_x_{box[0,:]}_{t[0]}.pt")
        x_vi=self.full_noisy_conv(x)
        #torch.save(x_vi,f"x_vi_{box[0,:]}_{t[0]}.pt")
        ...
        #torch.save(x,f"noise_{box[0,:]}_{t[0]}.pt")
        x=torch.cat([x_vi,x],dim=1)

        _,context=self.model(x, (torch.ones([b],device=x.device)).long(),context=None)
        out2=self.denoise_conv(x_vi)
        model_out,_ = self.model(x, t,context=context) # predict the noise that has been added to x_start or directly predict x_start from the noisy x, conditioned by the timestep t, cond   
        #torch.save(model_out,f"reco_{box[0,:]}_{t[0]}.pt")
        #model_out_n,intermediate_n = self.model(x_vi,torch.ones_like(t), cond = None) #vivek
        ...
        loss_deconv=self.loss_fn(out2,target,reduction='none') #vivek
        #loss_EL= torch.clamp((torch.pow(-torch.log((1-normalize_tensor(target-out2)).mean()), 0.3)), min=0, max=2)
        
        loss_patch = torch.zeros_like(loss)
           
        loss = reduce(loss, 'b ... -> b (...)', 'mean')
        loss_deconv=reduce(loss_deconv, 'b ... -> b (...)', 'mean') #vivek

        loss = loss * extract(self.p2_loss_weight, t, loss.shape)
        loss=EL_Loss(model_out,target)
        loss_deconv=EL_Loss(out2,target)
        if self.objective == 'pred_noise':
            #return loss.mean(), unnormalize_to_zero_to_one(x - extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * model_out)  #+loss_deconv.mean()
            #return 0.5*loss_deconv.mean()+loss_EL, unnormalize_to_zero_to_one(x - extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * model_out)
            return 0.5*loss_deconv+loss, unnormalize_to_zero_to_one(x - extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * model_out)
        else:
            #return loss.mean(), model_out #+loss_deconv.mean()
            #return 0.5*loss_deconv.mean()+loss_EL, model_out
            return 0.5*loss_deconv+loss, model_out

    def forward(self, img, t=None,train=True, *args, **kwargs):
        ...
        #t= (torch.ones([16],device=device)*500).long()
        img=(img - img.min()) / (img.max() - img.min())
        img = normalize_to_neg_one_to_one(img)
        return self.p_losses(img, t,train, *args, **kwargs)
