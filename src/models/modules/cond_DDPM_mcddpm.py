...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t,train, cond = None, noise = None, box=None, scale_patch=1, onlybox=False, mask=None):
        ...
        _,context=self.model(x, (torch.ones([b],device=x.device)).long(), cond = cond,context=None) #conditioning on timestep 0 image
        out2=self.denoise_conv(x_vi)
        model_out,_ = self.model(x, t, cond = cond,context=context) # predict the noise that has been added to x_start or directly predict x_start from the noisy x, conditioned by the timestep t, cond
        ...
        loss_deconv=self.loss_fn(out2,target,reduction='none') #deconvolution loss corresponding to bridge network
        loss_patch = torch.zeros_like(loss)
           
        loss = reduce(loss, 'b ... -> b (...)', 'mean')
        loss_deconv=reduce(loss_deconv, 'b ... -> b (...)', 'mean') #deconvolution loss corresponding to bridge network
        loss = loss * extract(self.p2_loss_weight, t, loss.shape)
        
        if self.objective == 'pred_noise':
            return loss.mean()+0.5*loss_deconv.mean(), unnormalize_to_zero_to_one(x - extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * model_out)
        else:
            return loss.mean()+0.5*loss_deconv.mean(), model_out

    def forward(self, img, t=None,train=True,cond=None, *args, **kwargs):
        ...
        return self.p_losses(img, t, train, cond, *args, **kwargs)
