...
def EL_Loss(x,y):
    ...
    b=x.shape[0]
    for i in range(b):
        diff=normalize_tensor(mse(x[i],y[i]))
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t,train, cond = None, noise = None, box=None, scale_patch=1, onlybox=False, mask=None):
        ...
        model_out,_ = self.model(x, t,context=None) # predict the noise that has been added to x_start or directly predict x_start from the noisy x, conditioned by the timestep t, cond   
