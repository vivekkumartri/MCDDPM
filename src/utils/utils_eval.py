...
def _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label_vol) :
        ...
        data_orig=normalize_tensor(data_orig)
        final_volume=normalize_tensor(final_volume)
       ...
       # Calculate Reconstruction errors with respect to anomal/normal regions
        ...
        if self.cfg['medianFiltering']:
            diff_volume_m = torch.from_numpy(apply_3d_median_filter(diff_volume.numpy().squeeze(),kernelsize=self.cfg.get('kernelsize_median',5))).unsqueeze(0) # bring back to tensor
        #torch.save(data_orig,f"/scratch/vivek.trivedi/bheeshm_bhaiya_experiment/qualitative_result/pddpm/original_image_{ID[0]}.pt")
        #torch.save(final_volume,f"/scratch/vivek.trivedi/bheeshm_bhaiya_experiment/qualitative_result/pddpm/generated_image_{ID[0]}.pt")
        #torch.save(data_seg,f"/scratch/vivek.trivedi/bheeshm_bhaiya_experiment/qualitative_result/pddpm/data_segmentation_{ID[0]}.pt")
        #torch.save(data_mask,f"/scratch/vivek.trivedi/bheeshm_bhaiya_experiment/qualitative_result/pddpm/mask_{ID[0]}.pt")
        # save image grid
        #if self.cfg['saveOutputImages'] :
            #log_images(self,diff_volume_m, data_orig, data_seg, data_mask, final_volume, ID)
        ...
        if self.cfg.evalSeg and self.dataset[0] not in self.healthy_sets: # only compute metrics if segmentation is available
            ...
            AUC, _fpr, _tpr, _threshs = 0.1,0.1,0.1,0.1
            AUPRC_m, _precisions, _recalls, _threshs = compute_prc(diff_volume_m.squeeze().flatten(),np.array(data_seg.squeeze().flatten()).astype(bool))
            ...
            bestDice, bestThresh = find_best_val(np.array(diff_volume.squeeze()).flatten(),  # threshold search with a subset of EvaluationSet
                                                ...
                                                max_val=0, 
                                                max_point=0)
            bestDice_m, bestThresh_m = find_best_val(np.array(diff_volume_m.squeeze()).flatten(),  # threshold search with a subset of EvaluationSet
                                                np.array(data_seg.squeeze()).flatten().astype(bool), 
                                                val_range=(0, np.max(np.array(diff_volume))),
                                                max_steps=10, 
                                                step=0, 
            ...
            if self.cfg["threshold"] == 'auto':
                ...
                diffs_thresholded = diff_volume > 0.5    
            if self.cfg["threshold"] == 'auto':
                diffs_thresholded_m = diff_volume_m > bestThresh_m
            else: # never used
                diffs_thresholded = normalize_tensor(diff_volume) > 0.5             
            ...
            self.eval_dict["Dice_thresh_0.1"].append(dice(np.array((normalize_tensor(diff_volume) > 0.1).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict["Dice_thresh_0.2"].append(dice(np.array((normalize_tensor(diff_volume) > 0.2).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict["Dice_thresh_0.3"].append(dice(np.array((normalize_tensor(diff_volume) > 0.3).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict["Dice_thresh_0.4"].append(dice(np.array((normalize_tensor(diff_volume) > 0.4).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict["Dice_thresh_0.5"].append(dice(np.array((normalize_tensor(diff_volume) > 0.5).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            ...
            self.eval_dict["Dice_thresh_0.1_m"].append(dice(np.array((normalize_tensor(diff_volume_m) > 0.1).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict["Dice_thresh_0.2_m"].append(dice(np.array((normalize_tensor(diff_volume_m) > 0.2).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict["Dice_thresh_0.3_m"].append(dice(np.array((normalize_tensor(diff_volume_m) > 0.3).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict["Dice_thresh_0.4_m"].append(dice(np.array((normalize_tensor(diff_volume_m) > 0.4).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict["Dice_thresh_0.5_m"].append(dice(np.array((normalize_tensor(diff_volume_m) > 0.5).squeeze()),np.array(data_seg.squeeze().flatten()).astype(bool)))
            self.eval_dict['BestDicePerVol_m'].append(bestDice_m)
            self.eval_dict['BestThresholdPerVol_m'].append(bestThresh_m)
            self.eval_dict['AUPRCPerVol_m'].append(AUPRC_m)
...
def _test_end(self) :
    # average over all test samples
        ...
        self.eval_dict['Dice_thresh_0.1_mean'] = np.nanmean(self.eval_dict['Dice_thresh_0.1'])
        self.eval_dict['Dice_thresh_0.1_std'] = np.nanstd(self.eval_dict['Dice_thresh_0.1'])
        self.eval_dict['Dice_thresh_0.2_mean'] = np.nanmean(self.eval_dict['Dice_thresh_0.2'])
        self.eval_dict['Dice_thresh_0.2_std'] = np.nanstd(self.eval_dict['Dice_thresh_0.2'])
        self.eval_dict['Dice_thresh_0.3_mean'] = np.nanmean(self.eval_dict['Dice_thresh_0.3'])
        self.eval_dict['Dice_thresh_0.3_std'] = np.nanstd(self.eval_dict['Dice_thresh_0.3'])
        self.eval_dict['Dice_thresh_0.4_mean'] = np.nanmean(self.eval_dict['Dice_thresh_0.4'])
        self.eval_dict['Dice_thresh_0.4_std'] = np.nanstd(self.eval_dict['Dice_thresh_0.4'])
        self.eval_dict['Dice_thresh_0.5_mean'] = np.nanmean(self.eval_dict['Dice_thresh_0.5'])
        self.eval_dict['Dice_thresh_0.5_std'] = np.nanstd(self.eval_dict['Dice_thresh_0.5'])
        
        self.eval_dict['Dice_thresh_0.1_mean_m'] = np.nanmean(self.eval_dict['Dice_thresh_0.1_m'])
        self.eval_dict['Dice_thresh_0.1_std_m'] = np.nanstd(self.eval_dict['Dice_thresh_0.1_m'])
        self.eval_dict['Dice_thresh_0.2_mean_m'] = np.nanmean(self.eval_dict['Dice_thresh_0.2_m'])
        self.eval_dict['Dice_thresh_0.2_std_m'] = np.nanstd(self.eval_dict['Dice_thresh_0.2_m'])
        self.eval_dict['Dice_thresh_0.3_mean_m'] = np.nanmean(self.eval_dict['Dice_thresh_0.3_m'])
        self.eval_dict['Dice_thresh_0.3_std_m'] = np.nanstd(self.eval_dict['Dice_thresh_0.3_m'])
        self.eval_dict['Dice_thresh_0.4_mean_m'] = np.nanmean(self.eval_dict['Dice_thresh_0.4_m'])
        self.eval_dict['Dice_thresh_0.4_std_m'] = np.nanstd(self.eval_dict['Dice_thresh_0.4_m'])
        self.eval_dict['Dice_thresh_0.5_mean_m'] = np.nanmean(self.eval_dict['Dice_thresh_0.5_m'])
        self.eval_dict['Dice_thresh_0.5_std_m'] = np.nanstd(self.eval_dict['Dice_thresh_0.5_m'])
        self.eval_dict['AUPRCPerVolMean_m'] = np.nanmean(self.eval_dict['AUPRCPerVol_m'])
        self.eval_dict['AUPRCPerVolStd_m'] = np.nanstd(self.eval_dict['AUPRCPerVol_m'])         
        self.eval_dict['BestDicePerVolMean_m'] = np.mean(self.eval_dict['BestDicePerVol_m'])
        self.eval_dict['BestDicePerVolStd_m'] = np.std(self.eval_dict['BestDicePerVol_m'])
